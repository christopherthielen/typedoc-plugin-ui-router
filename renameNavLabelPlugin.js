  ...
  function(require, exports) {
    ...
    var RenameNavLabelPlugin = (function(_super) {
      ...
      RenameNavLabelPlugin.prototype.initialize = function() {
        ...
        this.listenTo(this.owner, ((_a = {}), (_a[events_1.RendererEvent.BEGIN] = this.onBeginRenderer), _a));
        var _a;
      };
      ...
      RenameNavLabelPlugin.prototype.useIndexForGlobals = function(project) {
        function findGlobalReflection(ref) {
          ...
          if (ref.kind === models_1.ReflectionKind.Global) return ref;
      ...
      RenameNavLabelPlugin.prototype.renameNavigationItems = function(navigation) {
        ...
        navigation.children.forEach(function(item) {
          ...
          if (item.isGlobals && item.title === 'Globals') item.title = _this.labels.globals;
          if (item.isLabel && item.title === 'Internals') item.title = _this.labels.internals;
          if (item.isLabel && item.title === 'Externals') item.title = _this.labels.externals;
        });
      };
      RenameNavLabelPlugin.prototype.getNavigation = function() {
        var components = this.application.renderer.getComponents();
        var navigationPlugin = components.filter(function(c) {
          return c.componentName === 'navigation';
        })[0];
        if (!navigationPlugin) {
    ...
    RenameNavLabelPlugin = __decorate([components_1.Component({ name: 'RenameNavLabel' })], RenameNavLabelPlugin);
